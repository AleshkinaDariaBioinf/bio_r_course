...
cross.results <- apply(cross.validation, 1, function(validation){
  ...
  train.data <- select_data[train.sample, ]
  train.response <- select_data$age[train.sample]
  test.data <- select_data[validation, ]
...
Мы подобрали параметры для random forest для минимизации ошибки, с которыми модель работает лучше, чем с параметрами по умолчанию.